	...
	if(IsValidClient(iClient))
	{
		...
		if(iPrimary > -1)
		{
			for(int i = 0; i < MAXIMUM_WEAPON_SLOTS; i++)
			{
				...
				if(iWeapon > -1)
				{
					if(TF2Attrib_GetByDefIndex(iWeapon, PRIM_RELOAD_RATE_BONUS_ATTRIB_INDEX) != Address_Null)
					{
						float fBonus = TF2Attrib_GetValue(TF2Attrib_GetByDefIndex(iWeapon, PRIM_RELOAD_RATE_BONUS_ATTRIB_INDEX));
						TF2Attrib_SetByDefIndex(iPrimary, RELOAD_RATE_BONUS_ATTRIB_INDEX, fBonus);
					}
				}
			}
		}
